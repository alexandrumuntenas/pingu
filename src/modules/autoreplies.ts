        ...
        if (autoReply.propiedades.enviarEnEmbed.habilitado) {
          ...
          if (autoReply.propiedades.enviarEnEmbed.descripcion) {
            embed.setDescription(
              autoReply.propiedades.enviarEnEmbed.descripcion
            )
          } else embed.setDescription(autoReply.propiedades.respuesta)

          if (autoReply.propiedades.enviarEnEmbed.thumbnail) {
            embed.setThumbnail(
              autoReply.propiedades.enviarEnEmbed.thumbnail
            )
          }
...
export default new Module(
  'AutoReply',
  'Automatic customizable replies',
  [new EventHook('messageCreate', messageCreateHook, 'noPrefix')],
  { enabled: 'boolean' },
  { enabled: false }
)
