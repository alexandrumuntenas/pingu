...
module.exports = {
  ...
  runInteraction (interaction) {
    getCustomCommands(interaction.guild, customcommands => {
      if (customcommands.length === 0) return interaction.editReply({ embeds: [plantillas.estado(i18n.obtenerTraduccion(interaction.guild.preferredLocale, 'VIEWCUSTOMCOMMANDS::NOCUSTOMCOMMANDS'))] })
  ...
  runCommand (message) {
    getCustomCommands(message.guild, customcommands => {
      if (customcommands.length === 0) return message.reply({ embeds: [plantillas.estado(i18n.obtenerTraduccion(message.guild.preferredLocale, 'VIEWCUSTOMCOMMANDS::NOCUSTOMCOMMANDS'))] })
