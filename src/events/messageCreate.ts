...
import { ClientCommandsManager, ClientCooldownManager, ClientEventManager, ClientGuildManager, ClientMessageTemplate, ClientModuleManager, ClientUser } from '../client'
    ...
    ClientGuildManager.obtenerConfiguracionDelServidor(message.guild).then(configuracionDelServidor => {
      ...
      if ((message.content.startsWith(message.guildConfiguration.common.prefix) && message.content !== message.guildConfiguration.common.prefix) || message.content.startsWith(`<@${ClientUser.user.id}>`) || message.content.startsWith(`<@!${ClientUser.user.id}>`)) {
        ...
        if (message.guildConfiguration.interactions && message.guildConfiguration.interactions.enforceusage) {
          return message.reply({ embeds: [ClientMessageTemplate.error(obtenerTraduccion(message.guildConfiguration.language, 'INTERACTION-ENFORCEUSAGE'))] })
        }
        ...
        if (ClientCooldownManager.check(message.member, message.guild, message.command)) {
          if (ClientCommandsManager.has(message.command.name)) {
            if (message.command.module && ClientModuleManager.modulosDisponibles.includes(ClientModuleManager.getModulo(message.command.module)) && !message.guildConfiguration[message.command.module].enabled) return message.reply({ embeds: [ClientMessageTemplate.error(obtenerTraduccion(message.guild.preferredLocale, 'COMMAND::NOT_ENABLED'))] })

            if (message.command.permissions && !message.member.permissions.has(message.command.permissions, false)) return message.reply({ embeds: [ClientMessageTemplate.error(obtenerTraduccion(message.guild.preferredLocale, 'COMMAND::PERMERROR'))] })

            ClientCooldownManager.add(message.member, message.guild, message.command)

            return Object.prototype.hasOwnProperty.call(message.command, 'runCommand') ? message.command.runCommand(message) : message.reply({ embeds: [ClientMessageTemplate.error(obtenerTraduccion(message.guildConfiguration.common.language, 'COMMAND::ONLYINTERACTION'))] })
          }
          return ClientEventManager.ejecutarFuncionesDeTerceros('messageCreate', 'withPrefix', message)
        }
        return message.reply({ embeds: [ClientMessageTemplate.timeout(obtenerTraduccion(message.guildConfiguration.common.language, 'COOLDOWN', { COOLDOWN: humanizarTiempo(ClientCooldownManager.ttl(message.member, message.guild, message.command), { round: true, language: message.guildConfiguration.common.language || 'en', fallbacks: ['en'] }) }))] })
      }
