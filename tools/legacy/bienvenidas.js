...
module.exports = {
    ...
    execute (args, client, con, contenido, message, result) {
        ...
        if (message.guild.ownerID == message.author.id || message.member.hasPermission('ADMINISTRATOR')) {
            ...
            const roles_user = new Set()
            const usersss = result[0].welcome_roles
            const botssss = result[0].bienvenida_roles_bot
            if (usersss) {
                const role_user = usersss.split(',')
                ...
                message.channel.awaitMessages(m => m.author.id == message.author.id,
                    { max: 1 }).then(collected => {
                        if (collected.first().content === "y" || collected.first().content === "yes") {
                            const yes = "UPDATE `guild_data` SET `welcome_enabled` = '1' WHERE `guild_data`.`guild` = " + message.guild.id
                            con.query(yes)
                            message.channel.send(`<:pingu_check:876104161794596964> ${lan.toggle_message.response_b}`)
                            indice()
                        } else {
                            const no = "UPDATE `guild_data` SET `welcome_enabled` = '0' WHERE `guild_data`.`guild` = " + message.guild.id
                            con.query(no)
            ...
            function t_cartel () {
                ...
                message.channel.awaitMessages(m => m.author.id == message.author.id,
                    { max: 1 }).then(collected => {
                        if (collected.first().content === "y" || collected.first().content === "yes") {
                            const yes = "UPDATE `guild_data` SET `welcome_image` = '1' WHERE `guild_data`.`guild` = " + message.guild.id
                            con.query(yes)
                            message.channel.send(`<:pingu_check:876104161794596964> ${lan.toggle_cartel.response_b}`)
                            indice()
                        } else {
                            const no = "UPDATE `guild_data` SET `welcome_image` = '0' WHERE `guild_data`.`guild` = " + message.guild.id
                            con.query(no)
                ...
                message.channel.awaitMessages(m => m.author.id == message.author.id,
                    { max: 1 }).then(collected => {
                        if (collected.first().mentions.channels.first()) {
                            ...
                            const updatechannel = "UPDATE `guild_data` SET `welcome_channel` = '" + channel.id + "' WHERE `guild_data`.`guild` = " + message.guild.id
                    ...
                    { max: 1 }).then(collected => {
                        if (isInteger(collected.first().content)) {
                            if (parseInt(collected.first().content) <= 20 || parseInt(collected.first().content) >= 1) {
                                const updatemsg = "UPDATE `guild_data` SET `bienvenida_fondo` = '" + collected.first().content + "' WHERE `guild_data`.`guild` = " + message.guild.id
                                con.query(updatemsg)
