    ...
    execute (args, client, con, contenido, message, result) {
        ...
        if (message.guild.ownerID == message.author.id || message.member.hasPermission('ADMINISTRATOR')) {
                ...
                message.channel.awaitMessages(m => m.author.id == message.author.id,
                    { max: 1 }).then(collected => {
                        if (collected.first().content === "y" || collected.first().content === "yes") {
                            const yes = "UPDATE `guild_data` SET `leveling_enabled` = '1' WHERE `guild_data`.`guild` = " + message.guild.id
                            con.query(yes)
                            message.channel.send(`<:pingu_check:876104161794596964> ${lan.toggle_niveles.response_a}`)
                            indice()
                        } else {
                            const no = "UPDATE `guild_data` SET `leveling_enabled` = '0' WHERE `guild_data`.`guild` = " + message.guild.id
                            con.query(no)
            ...
            function u_mensaje () {
                ...
                message.channel.awaitMessages(m => m.author.id == message.author.id,
                    { max: 1 }).then(collected => {
                        const updatemsg = "UPDATE `guild_data` SET `leveling_rankup_message` = '" + emojiStrip(collected.first().content) + "' WHERE `guild_data`.`guild` = " + message.guild.id
                        con.query(updatemsg)
                ...
                message.channel.awaitMessages(m => m.author.id == message.author.id,
                    { max: 1 }).then(collected => {
                        if (collected.first().mentions.channels.first()) {
                            ...
                            const updatechannel = "UPDATE `guild_data` SET `leveling_rankup_channel` = '" + channel.id + "' WHERE `guild_data`.`guild` = " + message.guild.id
                    ...
                    { max: 1 }).then(collected => {
                        if (isInteger(collected.first().content)) {
                            if (parseInt(collected.first().content) <= 20 && parseInt(collected.first().content) >= 1) {
                                const updatemsg = "UPDATE `guild_data` SET `leveling_rankup_image_background` = '" + collected.first().content + "' WHERE `guild_data`.`guild` = " + message.guild.id
                                con.query(updatemsg)
                    ...
                    { max: 1 }).then(collected => {
                        if (isInteger(collected.first().content)) {
                            if (parseInt(collected.first().content) < 5 && parseInt(collected.first().content) > 0) {
                                const updatemsg = "UPDATE `guild_data` SET `leveling_rankup_difficulty` = '" + collected.first().content + "' WHERE `guild_data`.`guild` = " + message.guild.id
                                con.query(updatemsg)
