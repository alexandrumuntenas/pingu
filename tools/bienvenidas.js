    ...
    execute(client, con, Math, Jimp, downloader, webp, fs, pdf, moment, msi, emojiStrip, message, args, contenido, result, Intents, MessageEmbed, MessageReaction, MessageCollector, MessageAttachment, data) {
        if (message.guild.ownerID == message.author.id || message.member.hasPermission('ADMINISTRATOR')) {
            if (args[1]) {
                switch (args[1]) {
                    case 'canal':
                        if (message.mentions.channels.first()) {
                            ...
                            var sql = "UPDATE `servidores` SET `bienvenida_canal_id` = '" + channel.id + "' WHERE `servidores`.`guild` = " + data.server.id;
                            ...
                            message.channel.send(':information_source: Actualmente, el canal donde se notifican las bienvenidas es <#' + result[0].bienvenida_canal_id + '>. Para cambiar el canal, utilice el siguiente comando `' + data.server.prefix + 'bienvenidas canal <canal>`');
                        }
                        break;
                    case 'mensaje':
                        if (args[2]) {
                            var mensaje = message.content.replace(data.server.prefix + 'bienvenidas mensaje ', '');
                            var mensaje = emojiStrip(mensaje);
                            var sql = "UPDATE `servidores` SET `bienvenida_mensaje` = '" + mensaje + "' WHERE `servidores`.`guild` = " + data.server.id;
                            message.channel.send(':white_check_mark: Se ha actualizado el mensaje de bienvenida correctamente.');
                            con.query(sql);
                        } else {
                            message.channel.send(':information_source: Actualmente, el mensaje de bienvenida es `' + result[0].bienvenida_mensaje + '`. Para cambiar el mensaje, utilice el siguiente comando `' + data.server.prefix + 'bienvenidas mensaje <message>"`');
                        }
                        break;
                    case 'cartel':
                        ...
                        var sql = "UPDATE `servidores` SET `bienvenida_cartel` = '" + fin + "' WHERE `servidores`.`guild` = " + data.server.id;
                    ...
                    case 'enviar-mensaje':
                        ...
                        var sql = "UPDATE `servidores` SET `bienvenida_mensaje_activado` = '" + fin + "' WHERE `servidores`.`guild` = " + data.server.id;
                                    ...
                                    { max: 1 }).then(collected => {
                                        ...
                                        message.channel.awaitMessages(m => m.author.id == message.author.id,
                                            { max: 1 }).then(response2 => {
                                                ...
                                                var sql2 = "UPDATE `servidores` SET `bienvenida_canal_id` = '" + channel.id + "' WHERE `servidores`.`guild` = " + data.server.id;
                                                con.query(sql2)
                    ...
                    case 'dar-rol':
                        ...
                        var sql = "UPDATE `servidores` SET `bienvenida_roles_user` = '" + final + "' WHERE `servidores`.`guild` = " + data.server.id;
                        con.query(sql);
                ...
                var sql = "UPDATE `servidores` SET `bienvenida_activado` = '" + fin + "' WHERE `servidores`.`guild` = " + data.server.id;
