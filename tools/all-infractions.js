            ...
            if (result[0].moderador_activado != 0) {
                if (message.mentions.users.first()) {
                    ...
                    async function step1() {
                        con.query('SELECT * FROM `infracciones` WHERE user = \'' + user.id + '\' AND guild = \'' + data.server.id + '\'', function (err, result) {
                            if (err) throw err;
                            var pdfDoc = new pdf;
                            pdfDoc.pipe(fs.createWriteStream('./usuarios/moderacion/' + user.id + '_' + data.server.id + '.pdf'));
                            ...
                            pdfDoc.text("Servidor: " + data.server.name);
                            ...
                            pdfDoc.on('end', function (err) {
                                if (err) throw err;
                                var attachament = new MessageAttachment('./usuarios/moderacion/' + user.id + '_' + data.server.id + '.pdf');
                                message.author.send('Aquí está el reporte para el usuario `' + user.tag + '` en el servidor *' + data.server.name + '*', attachament);
                            })
                        });
                    }
