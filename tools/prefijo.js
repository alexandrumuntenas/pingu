    ...
    execute(args, client, con, contenido, downloader, emojiStrip, fetch, fs, global, Intents, Jimp, Math, message, MessageAttachment, MessageCollector, MessageEmbed, MessageReaction, moment, msi, pdf, result, webp) {
        if (message.guild.ownerID == message.author.id || message.member.hasPermission('ADMINISTRATOR')) {
            if (args[1]) {
                ...
                message.channel.send(":white_check_mark: Prefijo cambiado de \"" + global.server.prefix + "\" a \"" + args[1] + "\" correctamente :thumbsup:");
            } else {
                message.channel.send(':information_source: Falta un argumento en el comando. Uso: `' + global.server.prefix + 'prefijo <prefijo>`');
            }
        } else {
