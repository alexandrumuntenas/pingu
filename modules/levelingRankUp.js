module.exports = function (result, client, con, message, global) {
  ...
  con.query('SELECT * FROM `guildLevels` WHERE guild = ? AND user = ?', [message.guild.id, message.author.id], (err, result) => {
    ...
    if (Object.prototype.hasOwnProperty.call(result, 0)) {
      ...
      con.query('UPDATE `guildLevels` SET `experiencia` = ?, `nivel` = ? WHERE `user` = ? AND `guild` = ?', [exp, niv, message.author.id, message.guild.id], (err) => { if (err) console.log(err) })
    } else {
      con.query('INSERT INTO `guildLevels` (`user`, `guild`, `experiencia`) VALUES (?, ?, ?)', [message.author.id, message.guild.id, Math.round(Math.random() * (25 - 15) + 15)])
    }
  })
}
