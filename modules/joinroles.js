module.exports = {
  addJoinRole: (client, data, callback) => {
    ...
    client.pool.query(
      "INSERT INTO `guildJoinRoles` (`guild`, `roleID`) VALUES (?, ?)",
      [data.guild.id, data.role.id],
      (err) => {
  ...
  removeJoinRole: (client, data, callback) => {
    ...
    client.pool.query(
      "DELETE FROM `guildJoinRoles` WHERE `guild` = ? AND roleID = ?",
      [data.guild.id, data.role.id],
      (err) => {
        if (err) client.logError(err);
        if (callback && err) callback(err);
  ...
  fetchJoinRoles: (client, guild, callback) => {
    client.pool.query(
      "SELECT * FROM `guildJoinRoles` WHERE `guild` = ?",
      [guild.id],
      (err, rows) => {
        if (err) client.logError(err);
        if (rows && !err) {
