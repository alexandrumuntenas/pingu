...
module.exports = {
    ...
    if (!message.content.startsWith(message.database.guildPrefix)) {
      if (!talkedRecently.has(`${message.member.id}_${message.guild.id}`)) {
        ...
        getMember(client, message.member, (memberData) => {
          if (memberData) {
            let lvlExperience = parseInt(memberData.lvlExperience) + Math.round(Math.random() * (25 - 15) + 15);
            ...
            if (lvlExperience >= (((lvlLevel * lvlLevel) * dif) * 100)) {
              lvlExperience -= (((lvlLevel * lvlLevel) * dif) * 100);
              lvlLevel++;
              const messageToSend = message.database.levelsMessage.replace('{member}', `<@${message.member.id}>`).replace('{oldlevel}', `${lvlLevel - 1}`).replace('{newlevel}', `${lvlLevel}`);
            ...
            updateMember(client, message.member, { lvlLevel, lvlExperience });
          }
        });
      }
    }
  ...
  getLeaderboard(client, guild, callback) {
    client.pool.query('SELECT * FROM `memberData` WHERE guild = ? ORDER BY lvlLevel DESC, lvlExperience DESC LIMIT 25', [guild.id], (err, members) => {
