...
export default new Module('Farewell', 'Module for farewell messages', [
  new EventHook('guildMemberRemove', (member) => {
    ClientGuildManager.obtenerConfiguracionDelServidorPorModulo(member.guild, 'farewell').then((configuracionDelServidor) => {
      if (Object.prototype.hasOwnProperty.call(configuracionDelServidor, 'farewell') &&
                Object.prototype.hasOwnProperty.call(configuracionDelServidor.farewell, 'enabled')) {
        if (configuracionDelServidor.farewell.enabled) {
          if (!Object.prototype.hasOwnProperty.call(configuracionDelServidor.farewell, 'channel')) {
            return
          }
          try {
...
], { enabled: 'boolean', channel: 'string', message: 'string' }, { enabled: false, message: '**{user.tag}** just left the server' })
