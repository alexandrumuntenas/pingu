...
class CustomCommand {
  ...
  reply
  properties
  constructor (guild, name, properties) {
    this.guild = guild
    this.name = name
  ...
  executeCommand (message) {
    ...
    if (this.properties.sendInEmbed) {
      ...
      if (this.properties.sendInEmbed.description) {
        reply.content = reemplazarPlaceholdersConDatosReales(this.reply, message.member)
        embed.setDescription(reemplazarPlaceholdersConDatosReales(this.properties.sendInEmbed.description, message.member))
      } else {
        embed.setDescription(reemplazarPlaceholdersConDatosReales(this.reply, message.member))
      }
      ...
      reply.content = reemplazarPlaceholdersConDatosReales(this.properties.reply, message.member)
    }
    ...
    if (this.properties.sendDM) {
      try {
        reply.embeds = reply.embeds || [
          new EmbedBuilder().setDescription(reemplazarPlaceholdersConDatosReales(this.properties.reply, message.member))
        ]
    ...
    if (this.properties.sendChannel) {
      ...
      if (customChannelToSend && customChannelToSend.type === ChannelType.GuildText) { customChannelToSend.send(reply) } else { message.reply(reply) }
