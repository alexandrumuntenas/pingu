  ...
  guildFetchData(client, interaction.guild, async (guildData) => {
    ...
    if (client.commands.has(commandName)) {
      ...
      if (commandToExecute.permissions && !interaction.member.permissions.has(commandToExecute.permissions)) {
        messageBuilder.error.permissionerror(interaction, interaction.database.guildLanguage || 'en')
        return
      }
      if (cooldown.check(interaction.member, interaction.guild, commandToExecute)) {
        ...
        messageBuilder.error.cooldown(interaction, interaction.database.guildLanguage || 'en', (parseInt(cooldown.ttl(interaction.member, interaction.guild, commandToExecute)) - Date.now()))
      }
    } else {
