    ...
    if (interaction.database.suggestionsChannel) {
      ...
      await interaction.guild.channels.fetch(interaction.database.suggestionsChannel).then(channel => {
        channel.send({ embeds: [suggestionEmbed] }).then((_reply) => {
          createSuggestion(client, { suggestionId: makeId(5), suggestionContent: suggestion, suggestionAuthor: interaction.member.id, suggestionGuild: interaction.guild.id, suggestionMessage: _reply.id }, (status) => {
