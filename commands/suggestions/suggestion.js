    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case 'reject': {
        rejectSuggestion(interaction.member, interaction.options.getString('suggestion'), err => {
          ...
          return interaction.editReply({ embeds: [plantillas.conexito(i18n(locale, 'SUGGESTION::REJECT:SUCCESS', { SUGGESTIONID: interaction.options.getString('suggestion') }))] })
        })
        break
      }
      case 'addnote': {
        addNoteToSuggestion(interaction.member, interaction.options.getString('suggestion'), interaction.options.getString('note'), err => {
          ...
          return interaction.editReply({ embeds: [plantillas.conexito(i18n(locale, 'SUGGESTION::ADDNOTE:SUCCESS', { SUGGESTIONID: interaction.options.getString('suggestion') }))] })
        })
        break
      }
      case 'blacklist': {
        addUserToBlacklist(interaction.guild, interaction.options.getUser('user'), err => {
          if (err) return interaction.editReply({ embeds: [plantillas.error(i18n(locale, 'SUGGESTION::BLACKLIST:ERROR'))] })
          return interaction.editReply({ embeds: [plantillas.conexito(i18n(locale, 'SUGGESTION::BLACKLIST:SUCCESS', { USER: interaction.options.getUser('user').tag }))] })
        })
        break
      }
      case 'unblacklist': {
        removeUserFromBlacklist(interaction.guild, interaction.options.getUser('user'), err => {
          if (err) return interaction.editReply({ embeds: [plantillas.error(i18n(locale, 'SUGGESTION::UNBLACKLIST:ERROR'))] })
          return interaction.editReply({ embeds: [plantillas.conexito(i18n(locale, 'SUGGESTION::UNBLACKLIST:SUCCESS', { USER: interaction.options.getUser('user').tag }))] })
        })
        break
      }
      default: {
