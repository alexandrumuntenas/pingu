...
module.exports = {
  ...
  interactionData: new SlashCommandBuilder()
    ...
    .addSubcommand(sc => sc.setName('reject').setDescription('Reject a suggestion').addStringOption(input => input.setName('suggestion').setDescription('The suggestion or the message ID')))
    ...
    switch (interaction.options.getSubcommand()) {
      case 'approve': {
        approveSuggestion(interaction.member, interaction.options.getString('suggestion'), err => {
          ...
          return interaction.editReply({ embeds: [success(i18n(locale, 'SUGGESTION::APPROVE:SUCCESS'))] })
        })
        break
      }
      case 'reject': {
        rejectSuggestion(interaction.member, interaction.options.getString('suggestion'), err => {
          ...
          interaction.editReply({ embeds: [success(i18n(locale, 'SUGGESTION::REJECT:SUCCESS'))] })
        })
        break
      }
      case 'addnote': {
        addNoteToSuggestion(interaction.member, interaction.options.getString('suggestion'), interaction.options.getString('note'), err => {
          ...
          interaction.editReply({ embeds: [success(i18n(locale, 'SUGGESTION::ADDNOTE:SUCCESS'))] })
        })
        break
      }
      /* case 'blacklist': {
