...
module.exports = {
  ...
  runCommand (message) {
    const serverInfo = new MessageEmbed()
      ...
      .setDescription(`:medal: **${i18n(guild.preferredLocale, 'SERVER::EMBED:SERVERNAME')}**: ${message.guild.name}\n:id: **${i18n(guild.preferredLocale, 'SERVER::EMBED:SERVERID')}**: ${message.guild.id}\n:calendar: **${i18n(guild.preferredLocale, 'SERVER::EMBED:SERVERCREATIONDATE')}**: <t:${unixTime(message.guild.createdTimestamp)}>\n:crown: **${i18n(guild.preferredLocale, 'SERVER::EMBED:SERVEROWNER')}**: ${process.Client.users.cache.get(message.guild.ownerId).tag}\n<a:nitro:927222194034053161> **${i18n(guild.preferredLocale, 'SERVER::EMBED:NITROSTATUS')}**: ${message.guild.premiumTier} (${message.guild.premiumSubscriptionCount || '0'})`)
      .setFooter({ text: 'Powered by Pingu', iconURL: process.Client.user.displayAvatarURL() })
      .setTimestamp()
