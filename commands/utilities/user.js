	...
	runInteraction(locale, interaction) {
		const embed = new MessageEmbed()
			.setFooter({
				...
				iconURL: process.Client.user.displayAvatarURL(),
			})
			.setTimestamp();
		if (interaction.options.getUser('user')) {
			embed
				...
				.setDescription(
					`:crown: **${i18n(locale, 'USER::EMBED:USERTAG')}**: ${interaction.options.getUser('user').tag
			...
			embed
				...
				.setDescription(
					`:crown: **${i18n(locale, 'USER::EMBED:USERTAG')}**: ${interaction.user.tag
	...
	runCommand(locale, message) {
		...
		if (message.mentions.users.first()) {
			message.guild.members
				...
				.then(member => {
					embed
						...
						.setDescription(
							`:crown: **${i18n(locale, 'USER::EMBED:USERTAG')}**: ${message.mentions.users.first().tag
			...
			embed
				...
				.setDescription(
					`:crown: **${i18n(locale, 'USER::EMBED:USERTAG')}**: ${message.author.tag
