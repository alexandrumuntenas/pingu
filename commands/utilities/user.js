...
module.exports = {
  ...
  runInteraction (interaction) {
    ...
    if (interaction.options.getUser('user')) {
      embed
        .setTitle(i18n(guild.preferredLocale, 'USER::EMBED:TITLE', { USER: interaction.options.getUser('user').tag }))
        .setColor('#FFFFFF')
        .setThumbnail(interaction.options.getUser('user').displayAvatarURL({ dynamic: true }))
        .setDescription(`:crown: **${i18n(guild.preferredLocale, 'USER::EMBED:USERTAG')}**: ${interaction.options.getUser('user').tag}\n:tada: **${i18n(guild.preferredLocale, 'USER::EMBED:MEMBERNAME')}**: ${interaction.options.getMember('user').displayName}\n:id: **${i18n(guild.preferredLocale, 'USER::EMBED:USERID')}**: ${interaction.options.getMember('user').id}\n:calendar: **${i18n(guild.preferredLocale, 'USER::EMBED:ACCOUNTCREATIONDATE')}**: <t:${unixTime(interaction.options.getUser('user').createdTimestamp)}>\n:calendar: **${i18n(guild.preferredLocale, 'USER::EMBED:GUILDJOINDATE')}**: <t:${unixTime(interaction.options.getMember('user').joinedTimestamp)}>`)

      interaction.editReply({ embeds: [embed] })
    } else {
      embed
        .setTitle(i18n(guild.preferredLocale, 'USER::EMBED:TITLE', { USER: interaction.user.tag }))
        .setColor('#FFFFFF')
        .setThumbnail(interaction.user.displayAvatarURL({ dynamic: true }))
        .setDescription(`:crown: **${i18n(guild.preferredLocale, 'USER::EMBED:USERTAG')}**: ${interaction.user.tag}\n:tada: **${i18n(guild.preferredLocale, 'USER::EMBED:MEMBERNAME')}**: ${interaction.member.displayName}\n:id: **${i18n(guild.preferredLocale, 'USER::EMBED:USERID')}**: ${interaction.member.id}\n:calendar: **${i18n(guild.preferredLocale, 'USER::EMBED:ACCOUNTCREATIONDATE')}**: <t:${unixTime(interaction.user.createdTimestamp)}>\n:calendar: **${i18n(guild.preferredLocale, 'USER::EMBED:GUILDJOINDATE')}**: <t:${unixTime(interaction.member.joinedTimestamp)}>`)
