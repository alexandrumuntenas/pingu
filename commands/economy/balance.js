    ...
    if (interaction.database.economyEnabled !== 0) {
      getMember(client, interaction.member, (memberData) => {
        interaction.editReply({
          embeds: [
            Status(
              `**${interaction.user.tag}**: ${memberData.ecoBalance || 0} ${
                interaction.database.economyCurrency
              } ${interaction.database.economyCurrencyIcon}`
            ),
          ],
        });
      });
    ...
    if (message.database.economyEnabled !== 0) {
      getMember(client, message.member, (memberData) => {
        message.reply({
          embeds: [
            Status(
              `**${message.member.user.tag}**: ${memberData.ecoBalance || 0} ${
                message.database.economyCurrency
              } ${message.database.economyCurrencyIcon}`
            ),
          ],
        });
      });
