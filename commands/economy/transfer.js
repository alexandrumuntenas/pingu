      ...
      if (!interaction.options.getUser("user").bot) {
        if (parseInt(interaction.options.getNumber("amount")) > 0) {
          getMember(
            client,
            interaction.member,
            (fromUserInventoryAndBalance) => {
              if (
                fromUserInventoryAndBalance.ecoBalance >=
                ...
                getMember(client, toUser, (toUserInventoryAndBalance) => {
                  try {
                    updateMember(client, interaction.member, {
                      ecoBalance:
                        parseInt(fromUserInventoryAndBalance.ecoBalance) -
                        interaction.options.getNumber("amount"),
                    });
                    updateMember(client, toUser, {
                      ecoBalance:
                        parseInt(toUserInventoryAndBalance.ecoBalance) +
                        interaction.options.getNumber("amount"),
                    });
                    interaction.editReply({
                      embeds: [
                        Success(
                          i18n(locale, "TRANSFER::SUCCESS", {
                ...
                interaction.editReply({
                  embeds: [Error(i18n(locale, "TRANSFER::NOENOUGHMONEY"))],
                });
              }
            }
          );
          ...
          interaction.editReply({
            embeds: [Error(i18n(locale, "TRANSFER::INAVLIDAMOUNT"))],
          });
        }
      } else {
        interaction.editReply({
          embeds: [Error(i18n(locale, "TRANSFER::ISBOT"))],
        });
      }
    } else {
      interaction.editReply({
