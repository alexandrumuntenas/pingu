...
module.exports = {
  ...
  interactionData: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("productname")
        .setDescription("Enter the product name you want to checkout")
    ),
  executeInteraction(client, locale, interaction) {
    if (interaction.database.economyEnabled !== 0) {
          ...
          (productData) => {
            if (productData) {
              const productEmbed = new MessageEmbed()
                .setColor("#2F3136")
                .setAuthor({
                  name: `${interaction.guild.name} Shop`,
                  iconURL: interaction.guild.iconURL(),
                })
                .setTitle(productData.productName)
                .setImage(productData.productImage)
                .addField(
                  `${interaction.database.economyCurrencyIcon} ${i18n(
                    locale,
                    "PRICE"
                  )}`,
                  `${productData.productPrice} ${interaction.database.economyCurrency}`,
                  true
                )
                .addField(
                  `:robot: ${i18n(locale, "BUY")}`,
                  `\`/buy ${productData.productName}\``,
                  true
                )
                .setFooter({
                  text: "Powered by Pingu",
              ...
              if (productData.productDescription)
                productEmbed.setDescription(productData.productDescription);

              interaction.editReply({ embeds: [productEmbed] });
            } else {
              interaction.editReply({
                embeds: [Error(i18n(locale, "SHOP::NOTFOUND"))],
              });
            }
          }
        );
      ...
      interaction.editReply({
        embeds: [Error(i18n(locale, "COMMAND::NOTAVALIABLE"))],
      });
    }
  },
};
