...
module.exports = {
  ...
  executeInteraction(client, locale, interaction) {
    switch (interaction.options.getSubcommand()) {
      ...
      case 'delete': {
        client.pool.query('DELETE FROM `guildEconomyProducts` WHERE `productName` = ? AND `guild` = ?', [interaction.options.getString('name'), interaction.guild.id], (err) => {
          if (err) client.Sentry.captureException(err);
          genericMessages.success(interaction, getLocales(locale, 'PRODUCT_REMOVE_SUCCESS', { PRODUCT: interaction.options.getString('name') }));
        });
        break;
      }
    }
