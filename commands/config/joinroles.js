      ...
      switch (message.args[0]) {
        ...
        case 'add': {
          if (message.mentions.roles.first()) {
            message.mentions.roles.forEach((role) => addJoinRole(client, { guild: message.guild, role }, () => genericMessages.legacy.success(message, getLocales(locale, 'JOINROLES_ADD', { ROLE: role }))));
          } else {
        ...
        case 'remove': {
          if (message.mentions.roles.first()) {
            message.mentions.roles.forEach((role) => removeJoinRole(client, { guild: message.guild, role }, () => genericMessages.legacy.success(message, getLocales(locale, 'JOINROLES_REMOVE', { ROLE: role }))));
          } else {
            helpTray(message, locale);
          }
          break;
