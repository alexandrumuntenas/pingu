  ...
  executeLegacy(client, locale, message) {
    ...
    if (message.args[0]) {
      switch (message.args[0]) {
            ...
            message.channel.send({ content: `:arrow_right: ${getLocales(locale, 'AUTORESPONDER_CREATE_INSERTRIGGER')}` }).then((embedMenu) => {
              message.channel.awaitMessages({ filter, max: 1 }).then((collected) => {
                ...
                message.channel.awaitMessages({ filter, max: 1 }).then((collected) => {
                  ...
                  client.pool.query('INSERT INTO `guildAutoResponder` (`guild`, `autoresponderID`, `autoresponderTrigger`, `autoresponderResponse`) VALUES (?,?,?,?)', [message.guild.id, autoresponderId, autoresponderTrigger, autoresponderResponse], (err) => {
                    if (err) client.Sentry.captureException(err);
        ...
        case 'remove': {
          if (message.args[1]) {
            client.pool.query('DELETE FROM `guildAutoResponder` WHERE `autoresponderId` = ? AND `guild` = ?', [message.args[1], message.guild.id], (err) => {
              if (err) client.Sentry.captureException(err);
