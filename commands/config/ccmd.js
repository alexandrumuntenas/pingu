...
module.exports = {
  ...
  executeInteraction(client, locale, interaction) {
    switch (interaction.options.getSubcommand()) {
      case 'create': {
        client.pool.query('INSERT INTO `guildCustomCommands` (`guild`, `customCommand`, `messageReturned`) VALUES (?,?,?)', [interaction.guild.id, interaction.options.getString('command'), interaction.options.getString('response')], (err) => {
      ...
      case 'remove': {
        client.pool.query('DELETE FROM `guildCustomCommands` WHERE `customCommand` = ? AND `guild` = ?', [interaction.options.getString('command'), interaction.guild.id], (err) => {
          if (err) client.logError(err);
  ...
  executeLegacy(client, locale, message) {
    ...
    switch (message.args[0]) {
      case 'create': {
        ...
        client.pool.query('INSERT INTO `guildCustomCommands` (`guild`, `customCommand`, `messageReturned`) VALUES (?,?,?)', [message.guild.id, message.args[1], messageReturned], (err) => {
          if (err) client.logError(err);
      ...
      case 'remove': {
        client.pool.query('DELETE FROM `guildCustomCommands` WHERE `customCommand` = ? AND `guild` = ?', [message.args[1], message.guild.id], (err) => {
          if (err) client.logError(err);
