...
module.exports = {
  ...
  executeInteraction (client, locale, interaction) {
    if (interaction.options.getString('module')) {
      ...
      messageBuilder.success(interaction, getLocales(locale, 'P2ENMOD', { PMODULE: `\`${interaction.options.getString('module')}\`` }))
    } else {
      helpInteraction(interaction, locale)
    }
  },
  executeLegacy (client, locale, message) {
    if (message.args[0]) {
      ...
      messageBuilder.legacy.success(message, getLocales(locale, 'P2ENMOD', { PMODULE: `\`${message.args[0]}\`` }))
    } else {
      helpTray(message, locale)
    }
  }
...
function helpTray (message, locale) {
  messageBuilder.legacy.Info.help(message, locale, `${message.database.guildPrefix}p2enmod <module>`, ['welcomer', 'joinroles', 'farewell', 'moderation', 'levels', 'economy', 'autoresponder'])
}

function helpInteraction (message, locale) {
  messageBuilder.info.help(message, locale, '/p2dismod <module>', ['welcomer', 'joinroles', 'farewell', 'moderation', 'levels', 'economy', 'autoresponder'])
}
