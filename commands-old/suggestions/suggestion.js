...
module.exports = {
  ...
  runInteraction: async (client, locale, interaction) => {
    switch (interaction.options.getSubcommand()) {
      case 'approve': {
        approveSuggestion(client, { suggestionId: interaction.options.getString('id'), suggestionGuild: interaction.guild.id, suggestionSupervisor: interaction.member.id }, (status) => {
          ...
          const suggestionEmbed = new MessageEmbed().addField(i18n(locale, 'SUGGESTION::SUBMITTER'), `${suggestionAuthor.user.tag || 'Somebody :eyes:'} (${status.suggestionAuthor})`).addField(i18n(locale, 'SUGGESTION'), status.suggestionContent).setThumbnail(interaction.member.displayAvatarURL()).setColor('GREEN').setFooter({ text: `${i18n(locale, 'STATUS')}: ${i18n(locale, 'SUGGESTION::APPROVED')}\n${i18n(locale, 'SUGGESTION::REVISOR')}: ${interaction.member.user.tag} (${interaction.member.id})\nSuggestion ID: ${status.suggestionId}\nServer ID: ${interaction.guild.id}\nDate` }).setTimestamp()
      ...
      case 'reject': {
        rejectSuggestion(client, { suggestionId: interaction.options.getString('id'), suggestionGuild: interaction.guild.id, suggestionSupervisor: interaction.member.id }, (status) => {
          ...
          const suggestionAuthor = interaction.guild.members.resolve(status.suggestionAuthor)
          const suggestionEmbed = new MessageEmbed().addField(i18n(locale, 'SUGGESTION::SUBMITTER'), `${suggestionAuthor.user.tag || 'Somebody :eyes:'} (${status.suggestionAuthor})`).addField(i18n(locale, 'SUGGESTION'), status.suggestionContent).setThumbnail(interaction.member.displayAvatarURL()).setColor('RED').setFooter({ text: `${i18n(locale, 'SUGGESTION::STATUS')}: ${i18n(locale, 'SUGGESTION::REJECTED')}\n${i18n(locale, 'SUGGESTION::REVISOR')}: ${interaction.member.user.tag} (${interaction.member.id})\nSuggestion ID: ${status.suggestionId}\nServer ID: ${interaction.guild.id}\nDate` }).setTimestamp()
