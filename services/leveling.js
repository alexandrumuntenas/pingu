...
module.exports = function (result, client, con, message, global) {
    ...
    con.query("SELECT * FROM `guild_levels` WHERE guild = '" + message.guild.id + "' AND user = '" + message.author.id + "'", (err, result) => {
        if (result[0]) {
            ...
            const actualizardatos = "UPDATE `guild_levels` SET `experiencia` = '" + exp + "', `nivel` = '" + niv + "' WHERE `user` = '" + message.author.id + "' AND `guild` = '" + message.guild.id + "'"
            con.query(actualizardatos)
        } else {
            const newuser = "INSERT INTO `guild_levels` (`user`, `guild`) VALUES ('" + message.author.id + "', '" + message.guild.id + "')"
            con.query(newuser)
        }
    })
}
